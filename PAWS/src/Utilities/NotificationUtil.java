...
public class NotificationUtil {
	...
	public void generateNotif(String content, String type){
		...
		try{
		Connection conn = db.getConnection();
		PreparedStatement ps = conn.prepareStatement("INSERT INTO `notifications` (`content`,`dateCreated`,`status`,`type`)"
													+"VALUES (?, ?, ?, ? ); ");
...
System.out.println("EXPIRATION NOTIFS CREATED");
		...
		try{
			...
			PreparedStatement ps = conn.prepareStatement("SELECT `SPID` , `validThru` FROM `program-survey` WHERE `validThru` IS NOT NULL");
			ResultSet rs = ps.executeQuery();
...
//			System.out.println(content+" CONTENT OF EXPIRATION NOTIF");
	...
	private void getUnconfirmedSurveyList(ArrayList<Integer> filteredSurveyID){
		...
		try{
			Connection conn = db.getConnection();
			//SQL statement that checks if the current date is greater than the date of validity of a program's latest accreditation
			PreparedStatement ps = conn.prepareStatement("SELECT `surveyID` , `endDate`,`institutionID` FROM `surveys");
			ResultSet rs = ps.executeQuery();
			...
			while(rs.next()){
				...
				for(int i = 0; i<filteredSurveyID.size();i++){
					if(curDate-end_dateINT > 0 && filteredSurveyID.get(i) == rs.getInt(1)){
						...
						survTemp.setEndDate(rs.getString(2));
						survTemp.setInstitutionID(rs.getInt(3));
						survTemp.setInstitutionName(getInstName(rs.getInt(3)));
						System.out.println(i+" - "+survTemp.getEndDate()+" | "+survTemp.getInstitutionName());
	...
	private void generateUnconfirmedNotifs(ArrayList<Survey> surveyList){
		...
		for(int i = 0; i<surveyList.size(); i++){			
			String content = surveyList.get(i).getInstitutionName() + "'s Survey last '"+ formatDate(surveyList.get(i).getEndDate()) + "' is Unconfirmed";
	...
	private String getContentNames(int SPID){
		...
		try{
			Connection conn = db.getConnection();
			//SQL statement that checks if the current date is greater than the date of validity of a program's latest accreditation
			PreparedStatement ps = conn.prepareStatement("SELECT `degreeName`,`institutionID` FROM `school-program` WHERE SPID = ?");
			ps.setInt(1, SPID);
			ResultSet rs = ps.executeQuery();
			rs.next();
			name = rs.getString(1);
...
private String getSurveyDate(int surveyID){
		...
		try{
			Connection conn = db.getConnection();
			//SQL statement that checks if the current date is greater than the date of validity of a program's latest accreditation
			PreparedStatement ps = conn.prepareStatement("SELECT `endDate` FROM `surveys` Where `surveyID` = ?");
