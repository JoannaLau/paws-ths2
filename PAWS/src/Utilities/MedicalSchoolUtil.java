...
public class MedicalSchoolUtil {
	private DBUtil db;
	public MedicalSchoolUtil(){
		db = new DBUtil();
	}
	...
	public JSONArray getTertiaryJSON(int systemID){
		...
		try{
			...
			PreparedStatement ps = conn.prepareStatement("SELECT medID, name, city FROM `medicalschool` WHERE systemID = ?  ORDER BY `name`");
			ps.setInt(1, systemID);
			ResultSet rs = ps.executeQuery();
			while(rs.next()){
				job = new JSONObject();
				job.put("medID", rs.getInt(1));
				job.put("institutionName", rs.getString(2) + " - " + rs.getString(3));
				jArray.put(job);
				
			}
		} catch (Exception e){
			System.out.println("Error in MedicalSchoolUtil:getInstitutionsJSON()");
			e.printStackTrace();
		}
	...
	private String getSchoolSystemName(int ID){
		...
		try{
			...
			System.out.println("Error in MedicalSchoolUtil:getSchoolSystemName()");
			e.printStackTrace();
		}
	...
	public ArrayList<Institution> getInstitutions(){
		...
		try{
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM medicalschool ORDER BY `name`");
			ResultSet rs = ps.executeQuery();
			while(rs.next()){
			...
			System.out.println("Error in MedicalSchoolUtil:getInstitutions()");
			e.printStackTrace();
		}
	...
	public ArrayList<Institution> getSchoolSystemInstitutions(int systemID){
		...
		try{
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM medicalschool WHERE `systemID` = ?  ORDER BY `name`");
			ps.setInt(1, systemID);
			ResultSet rs = ps.executeQuery();
			while(rs.next()){
				//constructor is (int accreditorID, String name, String institution, String discipline, String primaryArea, 
			...
			System.out.println("Error in MedicalSchoolUtil:getSchoolSystemInstitutions()");
			e.printStackTrace();
		}
	...
	public Institution getInstitution(int instID){
		...
		try{
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM medicalschool a where a.medID = ?");
			ps.setInt(1,  instID);
			ResultSet rs = ps.executeQuery();
			while(rs.next()){
				temp = new Institution(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7), rs.getString(8), rs.getString(9), rs.getString(10), rs.getString(11), rs.getString(12), rs.getString(13), rs.getString(14), rs.getString(15), rs.getString(16), rs.getString(17), rs.getString(18));
			...
			System.out.println("Error in MedicalSchoolUtil:getInstitution()");
			e.printStackTrace();
		}
	...
	public void addInstitution(String ssName, String institutionName, String institutionAcronym, String address , String city , String country ,
		...
		try{
			...
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("INSERT INTO medicalschool (systemID, name, head, hPosition, hEmail, address, status, dateAdded, city, fax, contactPerson, contactPosition, contactNumber, website, country, contactEmail, acronym) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			System.out.println("Error in MedicalSchoolUtil:addInstitution()");
			e.printStackTrace();	
		}
	}
	...
	public void editInstitution(int institutionID, String ssName, String institutionName, String institutionAcronym, String address , String city , String country ,
		...
		try{
			...
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("UPDATE medicalschool SET systemID=?, name=?, head=?, hPosition=?, hEmail=?, address=?, status=?, dateAdded=?, city=?, fax=?, contactPerson=?, contactPosition=?, contactNumber=?, website=?, country=?, contactEmail=?, acronym=? WHERE medID=?");
			int ssID_int = Integer.parseInt(ssName);
			...
			System.out.println("Error in MedicalSchoolUtil:editInstitution()");
			e.printStackTrace();	
		}
	}
	...
	public void addProgramToInst(String specific, int generalID, int instID){
		try{
			...
			System.out.println("Error in MedicalSchoolUtil:addProgramToInstitution()");
			e.printStackTrace();	
		}
	...
	public void deleteInstitution(int institutionID){
		try{
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("DELETE from medicalschool WHERE medID = ?");
			ps.setInt(1, institutionID);
			ps.executeUpdate();
		} catch (Exception e){
			System.out.println("Error in MedicalSchoolUtil:deleteInstitution()");
			e.printStackTrace();
		}
	}
