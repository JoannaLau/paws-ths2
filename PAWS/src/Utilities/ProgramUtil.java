...
public class ProgramUtil {
	...
	private String getLastSurveyDate(int instID){
		...
		try{
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT `endDate` FROM `surveys` WHERE institutionID=?");
			ps.setInt(1, instID);
			ResultSet rs = ps.executeQuery();
			while(rs.next()){
	...
	public ArrayList<ProgramSurvey> getInstitutionProgramSurvey(int SPID){
		...
		try{
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT `PSID`, `surveyID`, `surveyType`, `validThru`, `boardApprovalDate` FROM `program-survey` WHERE SPID = ? AND currentDecisionBy = 'Board' ORDER BY `surveyID` DESC");
			ps.setInt(1, SPID);
			ResultSet rs = ps.executeQuery();
			while(rs.next()){
