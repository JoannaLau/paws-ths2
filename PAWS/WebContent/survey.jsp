...
function editAccreditor(accreditorID, institutionID, PSID, areaID, btn, surveyID){
	...
	add += "<hr>";
	add += '<div class="table-responsive" style="width:100%; float:right; height: 400px; overflow: auto;" id="contenthole">';
...
function formatDate(date) {
	...
	$(document).ready(function() {
		...
		$('#calendar').fullCalendar({
			...
			eventDrop: function(event, delta, revertFunc) {
		        ...
		        var currDate = $("#calendar").fullCalendar('getView').title;
		        goToDate(currDate);
		        $("#calendar").fullCalendar("refetchEvents");
		    	$("#calendar").fullCalendar("rerenderEvents");
	...
	if(year == "" || month == ""){
		...
		var noTime = $.fullCalendar.moment(year + '-' + month + '-01');
...
function saveType(PSID,pName, orig){
	...
	$.ajax({url: "UpdateSurveyType?PSID=" + PSID + "&type=" + type , success: function(result){
	...
	if(r == true){
		...
		$.ajax({ //CALLING ACCREDITORS WITH EXTRA CHECKING FOR AFFILIATION CONFLICTS
			  ...
			  success: function(data) {
	        		...
	        		errorDiv.innerHTML = "Successfully deleted survey!";
...
function loadPendings(){
	...
	$.getJSON("PendingSurveyLoader", function(data){
		$.each(data, function (key, value){
			var obj = $('<div class="fc-event ui-draggable ui-draggable-handle" style="margin: 3px; border-color:\'white\'; ">'+ value.title + '</div>').data('event', {
				title: value.title,
