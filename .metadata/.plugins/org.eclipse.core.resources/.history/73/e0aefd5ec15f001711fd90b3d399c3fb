...
public class BasicEdUtil {
	...
	public JSONArray getBasicEdJSON(int systemID){
		JSONArray jArray = new JSONArray();
		JSONObject job = new JSONObject();
		
		try{
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT beID, name, city FROM `basiceducation` WHERE systemID = ?  ORDER BY `name`");
			ps.setInt(1, systemID);
			ResultSet rs = ps.executeQuery();
			while(rs.next()){
				job = new JSONObject();
				job.put("beID", rs.getInt(1));
				job.put("institutionName", rs.getString(2) + " - " + rs.getString(3));
				jArray.put(job);
				
			}
		} catch (Exception e){
			System.out.println("Error in BasicEdUtil:getInstitutionsJSON()");
			e.printStackTrace();
		}
	...
	private String getSchoolSystemName(int ID){
		...
		try{
			...
			System.out.println("Error in BasicEdUtil:getSchoolSystemName()");
			e.printStackTrace();
		}
	...
	public ArrayList<Institution> getInstitutions(){
		...
		try{
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM basiceducation ORDER BY `name`");
			ResultSet rs = ps.executeQuery();
			while(rs.next()){
			...
			System.out.println("Error in BasicEdUtil:getInstitutions()");
			e.printStackTrace();
		}
	...
	public ArrayList<Institution> getSchoolSystemInstitutions(int systemID){
		...
		try{
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM basiceducation WHERE `systemID` = ?  ORDER BY `name`");
			ps.setInt(1, systemID);
			ResultSet rs = ps.executeQuery();
			while(rs.next()){
				//constructor is (int accreditorID, String name, String institution, String discipline, String primaryArea, 
			...
			System.out.println("Error in BasicEdUtil:getSchoolSystemInstitutions()");
			e.printStackTrace();
		}
	...
	public Institution getInstitution(int instID){
		...
		try{
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM basiceducation a where a.beID = ?");
			ps.setInt(1,  instID);
			ResultSet rs = ps.executeQuery();
			while(rs.next()){
				temp = new Institution(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7), rs.getString(8), rs.getString(9), rs.getString(10), rs.getString(11), rs.getString(12), rs.getString(13), rs.getString(14), rs.getString(15), rs.getString(16), rs.getString(17), rs.getString(18));
			...
			System.out.println("Error in BasicEdUtil:getInstitution()");
			e.printStackTrace();
		}
	...
	public void addInstitution(String ssName, String institutionName, String institutionAcronym, String address , String city , String country ,
		...
		try{
			...
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("INSERT INTO basiceducation (systemID, name, head, hPosition, hEmail, address, status, dateAdded, city, fax, contactPerson, contactPosition, contactNumber, website, country, contactEmail, acronym) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			System.out.println("Error in BasicEdUtil:addInstitution()");
			e.printStackTrace();	
		}
	}
	...
	public void editInstitution(int institutionID, String ssName, String institutionName, String institutionAcronym, String address , String city , String country ,
		...
		try{
			...
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("UPDATE basiceducation SET systemID=?, name=?, head=?, hPosition=?, hEmail=?, address=?, status=?, dateAdded=?, city=?, fax=?, contactPerson=?, contactPosition=?, contactNumber=?, website=?, country=?, contactEmail=?, acronym=? WHERE beID=?");
			int ssID_int = Integer.parseInt(ssName);
			...
			System.out.println("Error in BasicEdUtil:editInstitution()");
			e.printStackTrace();	
		}
	}
	...
	public void addProgramToInst(String specific, int generalID, int instID){
		try{
			...
			System.out.println("Error in BasicEdUtil:addProgramToInstitution()");
			e.printStackTrace();	
		}
	...
	public void deleteInstitution(int institutionID){
		try{
			Connection conn = db.getConnection();
			PreparedStatement ps = conn.prepareStatement("DELETE from basiceducation WHERE beID = ?");
			ps.setInt(1, institutionID);
			ps.executeUpdate();
		} catch (Exception e){
			System.out.println("Error in BasicEdUtil:deleteInstitution()");
			e.printStackTrace();
		}
	}
